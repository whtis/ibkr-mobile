...
TOKEN=$(grep ^API_TOKEN <repo>/backend/.env | cut -d= -f2)
...
|---|---|---|
| Live IBKR user (`<your-ibkr-username>`) | `backend/.env` `TWS_USERID` | IBKR Client Portal → Settings → Change Password |
...
| Field | Value |
|---|---|
| Trading Mode | **LIVE** (real account <account>) |
